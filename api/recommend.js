...
- 중고 제품, 리퍼비시(리퍼), 전시·반품 상품은 절대 추천하지 마라. 반드시 새 제품 기준으로 추천하라.
...
- 편지에서 긴 대시(—, –, ―)로 문장을 연결하지 마라. 문장이 끝나면 마침표를 쓰고 다음 문장을 이어라.
...
- 반드시 웹 검색으로 실제 존재하는 상품 페이지 URL을 확인해서 url에 넣어라. 실제 URL을 못 찾으면 url은 빈 문자열.
- url은 신뢰할 수 있는 대형 판매처(쿠팡, 네이버쇼핑/스마트스토어, 11번가, G마켓, SSG, 롯데온, 올리브영, 무신사, 카카오선물하기 등)만 사용하라. 개인 블로그·카페·중고 거래 링크는 절대 금지.
- 중고 제품, 리퍼비시(리퍼), 전시·반품 상품과 중고 거래 플랫폼(당근마켓, 번개장터, 중고나라 등)의 상품은 절대 선택하지 마라. 반드시 새 제품만 추천하라.
...
async function handler(req, res) {
  ...
  try {
    ...
    // 상품 정책 강제: 중고성 상품은 안전 선물로 교체, 신뢰되지 않는 구매 링크는 제거(검색 링크로 폴백)
    const policyActions = enforceProductPolicy(result);
    if (policyActions.length) log('product_policy.enforced', { actions: policyActions });
    ...
    // 모델이 긴 대시를 반환해도 저장 이미지에서 ㅡ처럼 보이지 않도록 마지막에 정리한다
    result.letter = normalizeLetterPunctuation(result.letter);
