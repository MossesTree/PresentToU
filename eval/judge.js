...
- 인용·환각 여부는 반드시 '[실제 입력 데이터]'의 대화 원문과 대조해서 판정하라. 대화 원문이 '(제공되지 않음)'인 케이스에서는 인용 검증 불가를 이유로 감점하지 마라.
...
// 심사자가 인용·환각을 대조할 수 있게 실제 요청 본문(대화 원문 포함)을 정리한다
// 대화가 아주 길면 검증에 필요한 앞뒤 위주로 자른다
function formatActualInput(body) {
  if (!body) return '(제공되지 않음)';
  const convo = body.conversation || '';
  const capped = convo.length > 15000 ? `${convo.slice(0, 12000)}\n…(중략)…\n${convo.slice(-3000)}` : convo;
  return `- 받는 사람: ${body.recipientName || '(미입력)'} / 성별: ${body.gender || '(미입력)'} / 관계: ${body.relationship || '(미입력)'}
- 선물 목적: ${body.purpose || '(미입력)'} / 예산: ${body.budgetAny ? '무관' : `${body.budgetMin}~${body.budgetMax}원`}
- 추가 정보: ${body.recipientInfo || '(없음)'}
- 대화 원문:
${capped || '(대화 없음)'}`;
}

// 케이스 정보를 심사용 사용자 프롬프트 한 덩어리로 조립한다
function buildJudgePrompt({ title, input, criteria, output, body }) {
